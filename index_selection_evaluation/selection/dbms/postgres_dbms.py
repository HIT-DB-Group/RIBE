...
class PostgresDatabaseConnector(DatabaseConnector):
    def __init__(self, db_name, autocommit=False):
        ...
        try:
            with open(self.config_path) as f:
                ...
                self.exist_index=index_config['exist_index']
    ...
    def create_connection(self):
        if self._connection:
            self.close()
        # print("dbname={}".format(self.db_name))
        # self._connection = psycopg2.connect("dbname={}".format(self.db_name))
        # self._connection = psycopg2.connect(f"dbname={self.db_name}, user={self.db_user}")
        self._connection = psycopg2.connect(database=self.db_name, user=self.db_user)
    ...
    # PostgreSQL requires an alias别名 for subqueries
    ...
    def _simulate_index(self, index):
        ...
        # "select * from hypopg_create_index( 'create index on table_name (index)')"
        statement = (
    ...
    def create_index_unique(self, index):
        time.sleep(1)
        table_name = index.table()
        name=f'act{int(time.time())}'
        self.idx_name_dict[index.index_idx()]=name
        statement = (
            f"create index {name} "
        ...
        size = self.exec_fetch(
            f"select relpages from pg_class c " f"where c.relname = '{name}'"
        )
        size = size[0]
    ...
    def drop_indexes(self):
        ...
        for index in indexes:
            index_name = index[0]
            # print(index) ('aka_title_pkey',)
            # print(index_name) aka_title_pkey
            if index_name in self.exist_index:
                continue
    ...
    def exec_query(self, query, timeout=None, cost_evaluation=False):
        ...
        try:
            ...
            result = plan["Actual Total Time"]*query.weight, plan
    ...
    def _get_cost(self, query):
        ...
        # 增加权重计算
        return total_cost*query.weight
