...
text2type_dict={'character':'varchar','text':'varchar','varchar':'varchar', 'date':'date', 'time':'time','char':'char','integer':'integer', 'decimal':'decimal'  }
...
class TableGenerator:
    ...
    def _read_column_names(self):
        ...
        for create_table in create_tables:
            ...
            for column in splitted[1].split(",\n"):
                l=re.sub(r'[ ]+',' ',column.lstrip().replace('\n',' ')).split(" ")
                
                name=l[0]
                if name == "primary":
                    continue
                
                column_type=text2type_dict[l[1].split('(')[0].lower()]
                column_object = Column(name,column_type)
    ...
    def _prepare(self):
        if self.benchmark_name == "tpch":
            ...
            self.directory = "./imdb"
            self.create_table_statements_file = "schema3.sql"
