...
from ..selection_algorithm import DEFAULT_PARAMETER_VALUES
from ..summary_selection_algorithm import SummarySelectionAlgorithm
...
class SummaryAnytimeAlgorithm(SummarySelectionAlgorithm):
    def __init__(self, database_connector, parameters=None):
        if parameters is None:
            parameters = {}
        SummarySelectionAlgorithm.__init__(
        ...
        self.name='smmary_anytime'

    def _calculate_best_indexes(self, workload):
        # Here the enumeration time is hard-coded by subtracting the compression algorithm time from the maximum end2end time
        algname2time={'nothing':180,'gsum':180,'isum app':180,'sample':180,'isum all':180}

        if self.cost_evaluation.compress_name is not None:
            logging.info(f'compress_name {self.cost_evaluation.compress_name} set the max running time {algname2time[self.cost_evaluation.compress_name]}')
            self.max_runtime_minutes=algname2time[self.cost_evaluation.compress_name]
        else:
            logging.info(f'compress_name is None, set the max running time {self.max_runtime_minutes}')
    ...
    def _simulate_and_evaluate_cost(self, workload, indexes):
        ...
        for query in workload.queries:
            ind=self.query2i_dict[query]
            cost,relevant_indexes=self._request_cache(query,indexes)
            if cost>=0:
                total_cost+=cost
            else:
                uncache_list.append(ind)
                uncache_key_list.append((query,relevant_indexes))
        if len(uncache_list)>0:
            cost_list=self.ws.get_cost_ml(uncache_list,indexes)

            for i,key in enumerate(uncache_key_list):
                self.cache[key]=cost_list[i]
